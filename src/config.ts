...
const schema = z.object({
  ...
  // 失败（error）灵感记录自动清理：0=失败后立即清除（默认）；>0=保留 N 小时后清除
  FAILED_RETENTION_HOURS: z.coerce.number().min(0).default(0),
  ...
  SCRAPE_TIMEOUT_MS: z.coerce.number().int().positive().default(8000),
...
export const llmConfigured = config.LLM_API_KEY.length > 0;