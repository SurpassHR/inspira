...
test('viewer：管理 GET 可读、写操作一律 403、提供商列表不含任何密钥形态', async () => {
  ...
  for (const [method, url, body] of [
    ['PUT', '/api/settings', { intervalMinutes: 60, enabled: true, themes: ['general'], activeThemes: ['general'], styles: ['anime'], activeStyles: ['anime'], kinds: ['image'], sources: ['original_idea'] }],
