...
const settings: InspirationSettings = {
  intervalMinutes: 60, coverRetryIntervalMinutes: 15, enabled: true, themes: ['自然', '科技'], activeThemes: ['自然'],
...
test('createSeed 只从已激活主题子集随机取（未勾选的不参与）', () => {
  const s: InspirationSettings = {
    intervalMinutes: 60, coverRetryIntervalMinutes: 15, enabled: true, themes: ['山水', '机甲'], activeThemes: ['山水'],
...
test('createSeed 风格从激活子集随机取；全部取消时 style 为 undefined（不回退全库）', () => {
  const base: InspirationSettings = {
    intervalMinutes: 60, coverRetryIntervalMinutes: 15, enabled: true, themes: ['a'], activeThemes: ['a'],
