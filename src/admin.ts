      ...
      <section class="sec" id="sec-gen">
        ...
        <div class="box ${can ? '' : 'locked'}">
          ...
          <div class="row">
            ...
            <div class="field" style="flex:0 0 148px">
              <label class="f">失败重试间隔（分钟）</label>
              <div class="stepper" id="retryStepper">
                <button class="step" data-d="-5" type="button" aria-label="减少 5 分钟">−</button>
                <div class="step-val" contenteditable="true" role="textbox" aria-label="失败重试间隔分钟"></div>
                <button class="step" data-d="5" type="button" aria-label="增加 5 分钟">+</button>
              </div>
              <div class="cb-hint">提示词生成失败自动重试的检查间隔，也是退避基准</div>
            </div>
          </div>
...
const retryStepper=makeStepper($('#retryStepper'));
...
function captureGenBase(){genBase={interval:intervalStepper.get(),coverRetry:coverRetryStepper.get(),coverDelay:coverDelayStepper.get(),retry:retryStepper.get(),enabled:enabledSw.get(),themes:themeEd.items,activeThemes:themeEd.active,styles:styleEd.items,activeStyles:styleEd.active,themeOverrides:themeEd.overrides,styleOverrides:styleEd.overrides,kinds:kinds.slice(),sources:sources.slice()};}
function genDirty(){
  if(!genBase)return false;
  return genBase.interval!==intervalStepper.get()||genBase.coverRetry!==coverRetryStepper.get()||genBase.coverDelay!==coverDelayStepper.get()||genBase.retry!==retryStepper.get()||genBase.enabled!==enabledSw.get()||
...
function applyGenBase(){
  if(!genBase)return;
  intervalStepper.set(genBase.interval);coverRetryStepper.set(genBase.coverRetry);coverDelayStepper.set(genBase.coverDelay);retryStepper.set(genBase.retry);enabledSw.set(genBase.enabled);
...
async function saveGenAsk(){
  try{
    await jf('/api/settings',{method:'PUT',headers:{'content-type':'application/json'},
      body:JSON.stringify({intervalMinutes:intervalStepper.get(),coverRetryIntervalMinutes:coverRetryStepper.get(),coverRetryDelaySeconds:coverDelayStepper.get(),retryIntervalMinutes:retryStepper.get(),enabled:enabledSw.get(),themes:themeEd.items,activeThemes:themeEd.active,styles:styleEd.items,activeStyles:styleEd.active,themeOverrides:themeEd.overrides,styleOverrides:styleEd.overrides,kinds,sources})});
    toast('已保存设置');
...
async function loadSettings(){
  try{
    ...
    intervalStepper.set(s.intervalMinutes);coverRetryStepper.set(s.coverRetryIntervalMinutes||15);coverDelayStepper.set(s.coverRetryDelaySeconds??30);retryStepper.set(s.retryIntervalMinutes||15);enabledSw.set(s.enabled);
