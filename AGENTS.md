...
- **可选生图**：图像类灵感在「LLM 配置 → 模型分配」指定了「生图」（task=`imagegen`）模型时，提示词就绪后自动生图（`src/llm.ts` 的 `generateImage()`，超时 `LLM_IMAGE_TIMEOUT_MS` 默认 180s）：**优先 OpenAI 兼容 `POST {base}/images/generations`（b64_json/url 双解析），失败（除 401）自动降级 chat completions** 并从回复提取图片（message.images / data URI / markdown / 裸 URL）——部分中转把 `gemini-*-image` 绑定在对话端点、`gpt-image-*` 绑定在 images 端点，自适应覆盖两类；4xx/空数据不重试（fatal 穿透），两路都失败时合并原因并**透出中转错误响应体摘要**（剔除密钥、截断 240 字符）；chat 兜底**回复含图片链接但下载失败时如实透出失败原因**（不误报「回复中未包含图片」）。**chat 兜底请求带 `stream:true`**——部分中转（如 flow2api）非流式会整体缓冲到上游生成完（单张可达 24 分钟）才响应，被前置 CDN 空闲超时（Cloudflare 100s）掐断成 524；流式则立即 200、心跳（注释行 / `reasoning_content`）续命、最终 chunk 携带 `![Generated Image](url)` markdown（`finish_reason='stop'`/`[DONE]` 收尾）。流式读取受双看门狗约束：总预算 `LLM_IMAGE_CHAT_TIMEOUT_MS`（默认 30min）+ 空闲 `LLM_IMAGE_CHAT_IDLE_MS`（默认 90s，须大于中转心跳间隔、小于 CDN 空闲上限）；中转忽略 `stream` 直接回 JSON 时按旧逻辑整体解析。产物封面图存 `DATA_DIR/images/{灵感id}.{ext}`、经 `GET /api/images/:name` 展示；画廊卡片一律展示 sharp 压缩的缩略图 `{id}.thumb.jpg`（宽 480px JPEG q72，约为原图 1/10 体积），点击卡片灯箱才加载原图，缩略图随封面生成自动产出、缺失时路由懒生成并落盘缓存、解码失败回退原图不阻断（`src/images.ts`）；生图严格按分配解析（**不回退**默认提供商/环境变量），未分配=静默不生图，失败只记 `coverError` 不影响 ready（`src/images.ts`）。
- **封面失败自动重试**（`src/cover-retry.ts`）：候选 = `status=ready` 且 `kind=image` 且有 `coverError` 无 `cover` 的条目（生成时未启用生图的缺封面条目**不**补）。检查间隔 = `settings.coverRetryIntervalMinutes`（默认 15，来源 `COVER_RETRY_INTERVAL_MINUTES`，后台「生成设置 → 封面补图间隔」可改并持久化；退避基准同步用它）。独立定时器每该值分钟一轮、指数退避（间隔×2^(失败次数-1)，上限 1h）、单条上限 `COVER_RETRY_MAX_ATTEMPTS`（默认 6，0=不限；达上限保留状态置 nextTryAt=∞——**不能删状态**，无状态会被视为到期再次重试）。轮内相邻两张之间等待 `settings.coverRetryDelaySeconds`（默认 30，来源 `COVER_RETRY_DELAY_SECONDS`，0=不等待；后台「生成设置 → 补图间隔（秒）」可改并持久化）。重试进度仅存内存，重启即清；成功写回 `cover` 并清除 `coverError`；`ImageGenNotConfiguredError`（未分配生图模型）整轮静默跳过不计次数。触发点：`startCoverRetryTimer()`（server.ts 启动定时器；**restartScheduler() 也会按当前设置重建定时器**，设置变更后新间隔即时生效）、`restartScheduler()`（提供商/分配/设置变更 → `resetCoverRetryState()` + 立即补一轮）、`POST /api/covers/retry`（admin，force+reset）。`retryFailedCovers()` 单飞保护，写回前重取 store 防止 update 复活已删条目。改重试逻辑必须跑 `npm test`（`cover-retry.test.ts`）。
- **提示词失败自动重试**（`src/inspiration-retry.ts`）：候选 = `status=failed` 的条目（图像/视频均覆盖）。检查间隔 = `settings.retryIntervalMinutes`（默认 15，来源 `RETRY_INTERVAL_MINUTES`，后台「生成设置 → 失败重试间隔」可改并持久化；退避基准同步用它）。独立定时器每该值分钟一轮、指数退避（间隔×2^(失败次数-1)，上限 1h）、单条上限 `RETRY_MAX_ATTEMPTS`（默认 6，0=不限；达上限保留状态置 nextTryAt=∞——**不能删状态**）。重试进度仅存内存，重启即清；重试用条目自身字段重建 seed（保留 id/主题/风格/比例）**重走完整生成管线**（素材 → 点子 → 提示词 → 封面），成功后条目恢复 ready 并清空 `error`（不新建卡片）；`llmReady()`=false（未配置 LLM）整轮静默跳过不计次数。触发点：`startInspirationRetryTimer()`（server.ts 启动定时器；**restartScheduler() 也会按当前设置重建定时器**）、`restartScheduler()`（提供商/分配/设置变更 → `resetInspirationRetryState()` + 立即补一轮）、`POST /api/inspirations/retry`（admin，force+reset）。`retryFailedInspirations()` 单飞保护，写回前重取 store 防止 update 复活已删条目；被 `FAILED_RETENTION_HOURS` 清理的失败条目不再重试。改重试逻辑必须跑 `npm test`（`inspiration-retry.test.ts`）。
...
- 组件实现位置：共享设计体系（CSS tokens）与客户端工具 `esc`/`fmt`/`ago` 单一来源在 `src/ui.ts`（SSR 内联字符串，画廊与后台两页共用，防止 XSS 转义不一致）；交互组件以小型 JS 工厂函数封装（如 `makeTextField`、`makeStepper`、`makeSwitch`、`makeSelect`）写在各自页面的内联 <script> 里（后台 `src/admin.ts`；公开画廊 `src/dashboard.ts`），纯 vanilla JS，无第三方依赖：
  ...
  - **override 注入重写（主题/风格自定义要求）**：给某主题/风格配的 override 模板（弹窗 `#ovrmodal` 编辑，`.txtarea` 多行 contenteditable）命中时，`generateInspiration`（src/generator.ts）由 `pickOverridePrompt()`（解析：仅 `kind='image'`，主题 override 优先、无则回退风格 override，均未命中返回 undefined 走正常 LLM）取模板，`renderOverrideTemplate()`（占位符 `{theme}/{style}/{aspect}/{title}/{idea}`，缺省值替换为空串）渲染后作为「自定义要求」**注入**图像提示词 LLM 请求（`deps.prompt` 的 `override` 参数 → `buildPromptPrompt` 的自定义要求块）——由 LLM 按 krea2 规范把点子与自定义要求**融合提炼、重写为新的最终提示词，不再机械拼接**（点子始终通过「创意点子」块传给 LLM，模板写不写 `{idea}` 都体现画面）。封面生图继续用重写后的最终提示词当入参。视频灵感与 `<Picture N>` 参考画面生图提示词不受影响。占位符与解析逻辑有单测守护（generator.test.ts：注入/优先级/未命中回退/视频豁免）——改 override 解析必须跑 `npm test`。
