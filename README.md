...
- **页面**：后台为左侧导航布局——**总览**（统计 / 调度 / LLM 生效目标 / 采集源健康 + 立即生成）/ **灵感**（筛选、详情、复制、单条删除 / 清空）/ **生成设置**（主题库、间隔、自动生成、类型与来源）/ **LLM 配置**（提供商 CRUD + 任务级模型分配）/ **采集源**（provider 白名单、自定义 URL、健康面板）/ **账号与权限**（用户增改删、在线会话吊销、安全审计日志；仅 admin 可见）。
